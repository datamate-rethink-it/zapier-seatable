...
zapier-files := $(zapier-package-files) $(build-source-files) $(build-test-files)
...
$(zapier-build-dir) : src/lint.cache.increment test/.increment $(zapier-build-paths)

$(zapier-build-paths) &: $(zapier-files)
	$(zapier) build
	$(zip) -ur build/build.zip src/authenticationFunctionRequire.js
	$(zip) -qd build/source.zip \
		.config/\* .idea/\* .nvmrc Makefile \*.md doc\* test\*
